...
async function _init({ profileId }, session) {
  await Coin.create([{
    profileId,
    type: 'signup_bonus',
    pulse: 0, nova: config.signupBonus.nova,
    pulseTotal: 0, novaTotal: config.signupBonus.nova,
    latest: true
  }], { session });
}

async function _deduct({ profileId, ref, type, countDeduct }, session) {

  let latestTxn = await Coin.findOne({ profileId, latest: true }).session(session);
